...
typography:
  ...
  card-title:{ fontFamily: "Inter", cqw: 2.3, weight: 550, lineHeight: 1.25, tracking: "-0.005em" }
  button:  { fontFamily: "Inter", cqw: 1.46, weight: 550, lineHeight: 1.0 }
  tag-upper:{ fontFamily: "Inter", cqw: 1.35, weight: 550, tracking: "0.18em", upper: true }
  kicker:  { fontFamily: "JetBrains Mono", px: 28, cqw: 1.46, weight: 550, tracking: "0.16em", upper: true }
  mono-label:{ fontFamily: "JetBrains Mono", px: 26, cqw: 1.35, weight: 550, tracking: "0.02em" }
  ...
  number-unit:{ fontFamily: "JetBrains Mono", cqw: 2.08, weight: 550, lineHeight: 1.0 }
...
components:
  ...
  number-lockup:
    ...
    description: "Hero stat — a Inter figure paired with a JetBrains Mono unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
    typography: "{typography.quote-pull} (Inter italic) + {typography.tag-upper} cite"
    description: "A commit message, a reviewer line, or the thesis. Inter italic, small Inter uppercase cite beneath."
  section-rule:
...
- **Fonts** — already set to **Inter** (display) / **Georgia** (body); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{400, 550, 600, 650, 700, 750}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
Three editorial voices, each in its own face: **Inter** carries every display moment — covers,
...
- **Inter** (sentence case, negative-tracked) for all display; **Inter** body/chrome; **JetBrains Mono** index + code.
...
- **Type** — Inter sentence-case display (negative-tracked); Inter 400 body; JetBrains Mono kickers (uppercase 0.16em, coral ✱) + code.
...
Mono `kicker`/`mono-label` in px) carries copy + chrome; the **display ramp** (Inter `headline` 4.6cqw
...
- **Inter display is sentence case** (NOT title case, NOT uppercase), weight 400, negative-tracked (−0.012..−0.028em); reach for **italic** when the line is a stance or a definition. **Inter body** sentence case weight 400. **JetBrains Mono** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.
...
- **number-lockup** — Inter figure + mono unit (the PR `+N / −M`, `200K`, file counts). **pull-quote** — Inter italic + cite (a commit message / reviewer line).
...
### 1 · Cover (identity · move: oversized Inter · cream)

**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, display-cover, lead, mono-label index. **Focal** a 2–3 line Inter `display-cover` (sentence case, ink) under a coral ✱ kicker. **Chrome** mono index strip (repo · branch). **Accent** the single coral ✱. **Fixed** Inter 400 sentence case, hairline, cream ground. **Free** title, the mono index, layout + how full the frame runs. **Density** free.

### 2 · Statement (statement · move: single Inter line · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}` for gravity). **Composes** kicker-spike, display, optional lead. **Focal** one 2-line Inter `display` carrying the change in a sentence — reach for **italic** if it's a stance. **Chrome** mono kicker. **Accent** none — the serif carries it (or one coral word). **Fixed** sentence-case serif. **Free** the line, ground, layout + density. **Density** free.
...
- Set all display in **Inter, sentence case**, negative-tracked; **Inter 400** body; **JetBrains Mono** kickers (uppercase, 0.16em, coral ✱) + code.
...
- Pair a Inter figure with a **mono unit** for every stat; render code via the **`code-*` blocks** on the navy surface.
...
- No uppercase or title-case Inter display; no sans headline; no serif label; no serif-set numeric unit.
...
- **Type** — Inter sentence-case display (negative-tracked); Inter 400 body; JetBrains Mono kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
